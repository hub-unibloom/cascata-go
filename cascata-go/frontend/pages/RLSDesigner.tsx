...
const RLSDesigner: React.FC<RLSDesignerProps> = ({ projectId, entityType, entityName, onBack }) => {
  ...
  const fetchDeepSchema = async () => {
    ...
    try {
        ...
        const myPolicies = policiesData.filter((p: any) => p.table === entityName);
  ...
  useEffect(() => {
      ...
      if (conflicting) {
          audit.score -= 40;
          audit.issues.push({ severity: 'critical', message: `Conflict: Policy "${conflicting.name}" already allows ALL access.` });
      }
