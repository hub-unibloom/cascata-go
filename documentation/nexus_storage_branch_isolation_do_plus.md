...
- **Branch URL Rewriting**: The `BranchRewriterInterceptor` has been completely unified to support branch-aware webhook endpoints out-of-the-box:
  - `https://{custom_domain}/webhook/branch/{branchName}/{pathSlug}` is automatically intercepted, setting the `X-Cascata-Env` header to `{branchName}` and rewriting the target routing path to `/webhook/{pathSlug}` before matching the Go-Chi routing tree.
  - `https://dash.unibloom.com.br/api/webhooks/in/{projectSlug}/branch/{branchName}/{pathSlug}` performs the exact same zero-overhead interception, rewriting the path to `/api/webhooks/in/{projectSlug}/{pathSlug}`.
  - This allows sandbox environments to receive live webhooks without slug collision or manually renaming webhook paths.
- **Frontend Synergy**: The `NexusArchitect` component automatically detects the active branch workspace environment (via `localStorage.getItem('cascata_env')`). If a sandbox branch is active (e.g. `devfull`), it renders a beautiful, copy-ready, branch-prefixed endpoint URL and constructs an exact branch-aware testing `cURL` command instantly.
